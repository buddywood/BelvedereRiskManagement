...
import { getIdentityRiskDashboardData } from "@/lib/actions/advisor-actions";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import IdentityRiskLoading from "./loading";

// Async component for data-dependent content
async function IdentityRiskContent() {
  const result = await getIdentityRiskDashboardData();

  if (!result.success) {
    return (
      <div className="space-y-6">
        <div className="text-center py-12">
          <p className="text-destructive text-sm">Error loading identity risk dashboard: {result.error}</p>
        </div>
      </div>
    );
  }
  ...
  // Check if any clients have completed identity risk assessments
      ...
      <div className="space-y-6">
        ...
        <div className="text-center py-12">
          <p className="text-muted-foreground">
            No clients have completed identity risk assessments yet.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-3xl">Identity Risk Portfolio</CardTitle>
          <CardDescription>
            View identity risk scores, risk levels, and assessment status for all assigned families.
              ...
              <tbody>
                {sortedClients.map((client) => (
                  <tr key={client.id} className="border-b">
                    ...
                    <td className="p-2">
                      {client.identityScore !== null
                        ? `${client.identityScore.toFixed(1)} / 10`
                        : 'Not assessed'
                      }
                    </td>
                    <td className="p-2">
...
export default function IdentityRiskDashboardPage() {
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <Suspense fallback={<IdentityRiskLoading />}>
        <IdentityRiskContent />
      </Suspense>
    </div>
  );
